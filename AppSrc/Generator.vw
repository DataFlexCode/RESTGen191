...
Object oGenerator is a dbView
    ...
    Procedure ReplaceNames Handle hoJson
        ...
        If (JsonType(hoJson) = jsonTypeObject) Begin
            ...
            Move (SizeOfArray(asRepl) - 1) to iLast
            
            For i from 0 to iLast
                
                If (HasMember(hoJson, asOrig[i])) Begin
                    Get Member        of hoJson asOrig[i] to hoMemb
                    Send RemoveMember of hoJson asOrig[i]
                    Send SetMember    of hoJson asRepl[i] hoMemb
    ...
    Procedure WriteStruct Handle hoJson String sName String sParent
        ...
        Integer  iChn i iLast iType iInd iArrType iMembs
        String   sMemb sPre
        ...
        Writeln channel iChn "// File: " sPre (UCFirst(Self, sName)) ".pkg generated by Unicorn InterGlobal's " (Module_Name(Self)) " program, " (String(CurrentDateTime()))
        Writeln channel iChn
// ToDo: write initial guff
//        Writeln channel iChn // initial guff
            ...
            Case Begin
                Case (iType = jsonTypeArray) 
                    ...
                    If (MemberCount(hoMemb)) Begin
                        ...
                        Case Begin
                            ...
                            Case (iArrType = jsonTypeNull)
                                ...
                                Writeln channel iChn "// ToDo: String substituted for null member in sample JSON"
                            ...
                            Case (iArrType = jsonTypeArray)
                                ...
                                Set piWarnings to (piWarnings(Self) + 1)
                                Writeln "// ToDo: Myulti dimentional array in sample JSON - String[] used instead"
                                Write channel iChn "String[] "
                                Case Break
                        Case End
                    ...
                    Else Begin
                        ...
                        Set piWarnings to (piWarnings(Self) + 1)
                        Writeln "// ToDo: Sample JSON had empty array, so used array of String instead, which may not be correct"
                        Write channel iChn (Repeat(" ", iInd)) "String[] "
                    End
                ...
                Case (iType = jsonTypeNull)
                    ...
                    Showln "     but this is probably incorrect"
                    Set piWarnings to (piWarnings(Self) + 1)
                    Writeln channel iChn "// ToDo: Substituted String for null member in sample JSON"
    ...
    Procedure Generate
        ...
        If (iType <> jsonTypeObject) Begin
            Send UserError "Outer JSON not an object" "No JSON Object"
            Send Destroy of hoJson
            Procedure_Return
        End
        ...
        Get Seq_New_Channel to iChn
        Append_Output channel iChn (psPath(Self) + "\" + sName + ".pkg")


        Writeln channel iChn "Object oStructHandler_" sName " is a cObject"
        WriteLn
        Writeln (Repeat(" ", iInd)) "Procedure StructToJsonUChar " sName " stValue UChar[] ByRef ucaData"
        Writeln (Repeat(" ", iInd * 2)) "Handle hoJson"
        Writeln 
        Writeln (Repeat(" ", iInd * 2)) "Get Create (RefClass(cJsonObject)) to hoJson"
        Writeln (Repeat(" ", iInd * 2)) "Send DataTypeToJson of hoJson stValue"
        Writeln (Repeat(" ", iInd * 2)) "Get StringifyUtf8 of hoJson to ucaData"
        Writeln (Repeat(" ", iInd * 2)) "Send Destroy of hoJson"
        Writeln (Repeat(" ", iInd)) "End_Procedure"
        Writeln
        Writeln (Repeat(" ", iInd )) "Procedure JsonUCharToStruct UChar[] ucaData " sName " ByRef stValue String ByRef sErr"
        Writeln (Repeat(" ", iInd * 2)) "Handle hoJson"
        Writeln (Repeat(" ", iInd * 2)) "Boolean bOK"
        Writeln
        Writeln (Repeat(" ", iInd * 2)) "Get Create (RefClass(cJsonObject)) to hoJson"
        Writeln (Repeat(" ", iInd * 2)) "Set pbRequireAllMembers of hoJson to False"
        Writeln (Repeat(" ", iInd * 2)) "Get ParseUtf8 of hoJson ucaData to bOK"
        Writeln
        Writeln (Repeat(" ", iInd * 2)) "If bOK ;"
        Writeln (Repeat(" ", iInd * 3)) "Get JsonToDataType of hoJson to stValue"
        Writeln (Repeat(" ", iInd * 2)) "Else ;"
        Writeln (Repeat(" ", iInd * 3)) "Get psParseError of hoJson to sErr"
        Writeln (Repeat(" ", iInd * 2)) ""
        Writeln (Repeat(" ", iInd * 2)) "Send Destroy of hoJson"
        Writeln (Repeat(" ", iInd)) "End_Procedure"
        Writeln
